-- ============================================================================
-- create_order v9 — payment-provider aware.
--
-- Generated from v8 (20260817110000) so the entire pricing body is byte-for-
-- byte the same; only these things changed:
--   * new p_payment_provider parameter ('paypal' or NULL)
--   * the order records payment_status / payment_provider
--   * notified_at stays NULL for an unpaid PayPal order, so no email goes out
--     until record_paypal_capture stamps it
--   * the return value carries payment_status
--
-- The 5-argument v8 is dropped first. Leaving both would make a named-argument
-- call from PostgREST ambiguous, and PostgREST resolves overloads by argument
-- NAMES — the failure would look like a 404 on the RPC, not a type error.
-- Re-runnable.
-- ============================================================================
