-- ============================================================================
-- create_order v11 — a service can set its own per-dish minimum.
--
-- Generated from v10 (20260830000200) with ONE behavioural change: the per-dish
-- floor now falls back through the dish's CATEGORY before the store default —
--   coalesce(menu_items.min_qty, categories.min_qty_per_dish, store default)
-- — so Party Trays (sold by the piece) stops demanding 15 of every dish while
-- Check-a-Lunch keeps the store floor. The join is LEFT so a dish with no
-- category is treated exactly as it was before.
--
-- Same 6-argument signature as v10 — grants carry over, and no drop is needed.
-- Re-runnable.
-- ============================================================================
...
begin
  ...
  -- their floor — an extra is still a kitchen batch. v11 resolves that floor
  -- per dish, then per service, then store-wide. First violation (alphabetical,
  ...
  if v_boxes > 0 then
    ...
    select mi.name,
           coalesce(mi.min_qty, c.min_qty_per_dish, v_min_dish) as required,
           t.have
      into v_viol
      ...
      -- v11: the dish's service may set its own floor. LEFT so a dish with no
      -- category still falls through to the store default, as before.
      left join public.categories c on c.id = mi.category_id
     where coalesce(mi.min_qty, c.min_qty_per_dish, v_min_dish) > 0
       and t.have < coalesce(mi.min_qty, c.min_qty_per_dish, v_min_dish)
     order by mi.name
     limit 1;
