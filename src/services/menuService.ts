...
export interface MenuItem {
  ...
  class MenuService {
    ...
    constructor(baseUrl = "http://localhost:5678/webhook/menu") {
    ...
    async getTypeMenuData(
      ...
      try {
        ...
        return result.data as { "check-a-lunch": MenuItem[]; "fun-boxes": MenuItem[] };
      } catch (error) {
    ...
    async getCategoryTypeMenuData(
      ...
      try {
        const response = await fetch(`${this.baseUrl}?category=${category}&type=${type}`, {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
            Accept: "application/json",
